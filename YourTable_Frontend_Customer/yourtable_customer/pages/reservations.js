...
export async function getServerSideProps(context) {
  ...
  if (!session ) {
    ...
    console.log(session)
      const res = await fetch(`http://34.139.40.48/reservation/`+session.ID);
...
function reservations({ reservations, successful, session }) {
              ...
              <table className="min-w-max w-full table-auto">
                <thead>
                  <tr className="bg-gray-200 text-gray-600 uppercase text-sm leading-normal">
                    ...
                    <th className="py-3 px-6 text-center">Personen</th>
